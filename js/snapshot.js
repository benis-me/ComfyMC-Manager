  ...
  if (SnapshotManager.instance) {
    try {
      ...
      if (response.status == 400) {
        app.ui.dialog.show(
          `Restore snapshot failed: ${target.title} / ${exception}`
        )
      ...
      SnapshotManager.instance.updateMessage(
        "<BR>To apply the snapshot, please <button id='cm-reboot-button' class='cm-small-button'>RESTART</button> ComfyUI. And refresh browser.",
        'cm-reboot-button'
      )
    }
  }
...
async function remove_snapshot(target) {
  if (SnapshotManager.instance) {
    try {
      const response = await api.fetchApi(`/snapshot/remove?target=${target}`, {
        cache: 'no-store',
      })
      if (response.status == 400) {
        app.ui.dialog.show(
          `Remove snapshot failed: ${target.title} / ${exception}`
        )
        app.ui.dialog.element.style.zIndex = 10010
      }
...
export class SnapshotManager extends ComfyDialog {
  ...
  startRestore(target) {
    ...
    self.updateMessage(
      `<BR><font color="green">Restore snapshot '${target.name}'</font>`
    )
