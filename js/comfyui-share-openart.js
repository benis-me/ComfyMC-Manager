...
export class OpenArtShareDialog extends ComfyDialog {
  ...
  async readKey() {
    ...
    try {
      key = await api
        .fetchApi(`/manager/get_openart_auth`)
  ...
  createButtons() {
    ...
    const linkSection = $el(
      ...
      [this.communityLink, this.getAPIKeyLink]
    )
  ...
  async share() {
    ...
    const current_snapshot = await api
      .fetchApi(`/snapshot/get_current`)
      .then((response) => response.json())
      .catch((error) => {
        // console.log(error);
      })
