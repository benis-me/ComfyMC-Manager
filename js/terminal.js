    ...
    class TerminalNode {
      ...
      constructor() {
        ...
        ComfyWidgets.BOOLEAN(
          this,
          'mode',
          ['', { default: true, label_on: 'Logging', label_off: 'Stop' }],
          app
        )
        ComfyWidgets.INT(
          this,
          'lines',
          ['', { default: 500, min: 10, max: 10000, steps: 1 }],
          app
        )
        ...
        if (terminal_node) {
          try {
            terminal_node.widgets[0].value =
              'The output of this node is disabled because another terminal node has appeared.'
          } catch {}
        }
        terminal_node = this
      }
...
function terminalFeedback(event) {
  if (terminal_node) {
    ...
    terminal_node.widgets[0].value = [...terminal_node.logs]
      .reverse()
      .join('')
      .trim()
  }
