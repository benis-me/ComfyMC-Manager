...
async function getCustomNodes() {
  ...
  const response = await api.fetchApi(
    `/customnode/getlist?mode=${mode}${skip_update}`
  )
    ...
    if (node_to_extensions_map[node].length > 1) {
      for (let i in node_to_extensions_map[node]) {
        ...
        for (let j in node_to_extensions_map[node]) {
          ...
          if (extension != extension2) l.push([node, extension2])
        }
      }
    }
  }
...
export class CustomNodesInstaller extends ComfyDialog {
  ...
  startInstall(target) {
    ...
    self.updateMessage(
      `<BR><font color="green">Installing '${target.title}'</font>`
    )
  }
  ...
  apply_searchbox(data) {
    ...
    for (let i in this.grid_rows) {
      ...
      if (keyword == '') this.grid_rows[i].control.style.display = null
  ...
  async filter_missing_node(data) {
    ...
    const group_nodes =
      workflow.extra && workflow.extra.groupNodes
  ...
  invalidate_checks(is_checked, install_state) {
    if (is_checked) {
      ...
      for (let i in this.grid_rows) {
        let checkbox = this.grid_rows[i].checkbox
        if (checkbox.check) return // do nothing
      }
  ...
  async createGrid() {
    ...
    this.checkbox_all.checked = false
    this.checkbox_all.disabled = true
    this.checkbox_all.addEventListener('change', function () {
      self.check_all.call(self, self.checkbox_all.checked)
    })
    ...
    if (this.data)
      for (var i = 0; i < this.data.length; i++) {
        ...
        checkbox.addEventListener('change', function () {
          self.invalidate_checks.call(self, checkbox.checked, data.installed)
        })
        ...
        if (data.installed == 'Fail')
          data3.innerHTML =
            '&nbsp;<font color="BLACK"><B>(IMPORT FAILED)</B></font>' +
            data3.innerHTML
        ...
        if (conflicts) {
          let buf =
            '<p class="cm-conflicted-nodes-text"><B><font color="BLACK">Conflicted Nodes:</FONT></B><BR>'
        ...
        else dataRow.style.backgroundColor = 'var(--bg-color)'
  ...
  createHeaderControls() {
    ...
    this.search_box = $el(
      ...
      {
        type: 'text',
        id: 'manager-customnode-search-box',
    ...
    if (this.channel != 'default') {
      channel_badge = $el('span', { id: 'cm-channel-badge' }, [
        `Channel: ${this.channel} (Incomplete list)`,
      ])
    } else {
    }
  ...
  async createBottomControls() {
    ...
    this.message_box = $el('div', { id: 'custom-installer-message' }, [
      $el('br'),
      '',
    ])
