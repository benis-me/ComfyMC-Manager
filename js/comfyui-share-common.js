      ...
      if (node.hasOwnProperty('widgets') && Array.isArray(node.widgets)) {
        ...
        for (let j = 0; j < node.widgets.length; j++) {
          if (node.widgets[j].type === 'image') {
            ...
            if (
              parsedURLVals.hasOwnProperty('filename') &&
              parsedURLVals.hasOwnProperty('subfolder') &&
              parsedURLVals.hasOwnProperty('type') &&
              parsedURLVals.hasOwnProperty('format')
            ) {
              ...
              potential_outputs.push({
                ...
                node_id: node.id,
                output: {
                  filename: parsedURLVals.filename,
                  subfolder: parsedURLVals.subfolder,
                  value: widgetValue,
                  format: parsedURLVals.format,
                },
              })
            }
            ...
            if (
              ...
              potential_outputs.push({
                ...
                output: {
                  filename: parsedURLVals.filename,
                  subfolder: parsedURLVals.subfolder,
                  value: `/view?filename=${parsedURLVals.filename}&subfolder=${parsedURLVals.subfolder}&type=${parsedURLVals.type}&format=${parsedURLVals.format}`,
                  format: parsedURLVals.format,
                },
              })
            }
...
export class ShareDialog extends ComfyDialog {
  ...
  createButtons() {
    ...
    this.is_nsfw_checkbox = $el(
      'input',
      { type: 'checkbox', id: 'is_nsfw' },
      []
    )
    ...
    this.matrix_destination_checkbox = $el(
      'input',
      { type: 'checkbox', id: 'matrix_destination' },
      []
    )
    const matrix_destination_checkbox_text = $el('label', {}, [
      ' ComfyUI Matrix server',
    ])
    ...
    this.comfyworkflows_destination_checkbox = $el(
      'input',
      { type: 'checkbox', id: 'comfyworkflows_destination' },
      []
    )
    const comfyworkflows_destination_checkbox_text = $el('label', {}, [
      ' ComfyWorkflows.com',
    ])
    this.comfyworkflows_destination_checkbox.style.color = 'var(--fg-color)'
    this.comfyworkflows_destination_checkbox.checked =
      this.share_option !== 'matrix'

    this.matrix_homeserver_input = $el(
      'input',
      {
        type: 'text',
        id: 'matrix_homeserver',
        placeholder: 'matrix.org',
        value: ShareDialog.matrix_auth.homeserver || 'matrix.org',
      },
      []
    )
    this.matrix_username_input = $el(
      'input',
      {
        type: 'text',
        placeholder: 'Username',
        value: ShareDialog.matrix_auth.username || '',
      },
      []
    )
    this.matrix_password_input = $el(
      'input',
      {
        type: 'password',
        placeholder: 'Password',
        value: ShareDialog.matrix_auth.password || '',
      },
      []
    )

    this.cw_sharekey_input = $el(
      'input',
      {
        type: 'text',
        placeholder: 'Share key (found on your profile page)',
        value: ShareDialog.cw_sharekey || '',
      },
      []
    )
    this.cw_sharekey_input.style.width = '100%'

    this.credits_input = $el(
      'input',
      {
        type: 'text',
    ...
    this.share_finalmessage_container = $el(
      'div.cm-menu-container',
      {
      ...
      [
        $el('div.cm-menu-column', [
          ...
          $el('button', {
            ...
            onclick: () => {
              // Reset state
              this.matrix_destination_checkbox.checked =
                this.share_option === 'matrix'
              this.comfyworkflows_destination_checkbox.checked =
                this.share_option !== 'matrix'
        ...
        $el('div.cm-menu-column', [
          ...
          $el(
            ...
            [
              ...
              $el('p', { size: 3, color: 'white' }, [
                'When provided, your art will be saved to your account.',
              ]),
    ...
    try {
      api
        .fetchApi(`/manager/get_matrix_auth`)
        .then((response) => response.json())
        .then((data) => {
          ShareDialog.matrix_auth = data
          this.matrix_homeserver_input.value =
            ShareDialog.matrix_auth.homeserver
    ...
    try {
      ...
      api
        .fetchApi(`/manager/get_comfyworkflows_auth`)
        .then((response) => response.json())
        .then((data) => {
    ...
    this.share_button.onclick = async () => {
      ...
      if (destinations.includes('matrix')) {
        let definedMatrixAuth =
          !!this.matrix_homeserver_input.value &&
          !!this.matrix_username_input.value &&
          !!this.matrix_password_input.value
      ...
      if (
        destinations.includes('comfyworkflows') &&
        !this.cw_sharekey_input.value &&
        !confirm(
          'You have NOT set your ComfyWorkflows.com share key. Your art will NOT be connected to your account (it will be shared anonymously). Continue?'
        )
      ) {
        return
      }
      ...
      if (response_json.comfyworkflows.url) {
        this.final_message.innerHTML =
          "Your art has been shared: <a href='" +
          response_json.comfyworkflows.url +
          "' target='_blank'>" +
          response_json.comfyworkflows.url +
          '</a>'
        if (response_json.matrix.success) {
          this.final_message.innerHTML +=
            "<br>Your art has been shared in the ComfyUI Matrix server's #share channel!"
        }
      } else {
        if (response_json.matrix.success) {
          this.final_message.innerHTML =
    ...
    const new_radio_buttons = $el(
      ...
      potential_outputs.map((output, index) => {
        ...
        if (output.type === 'image' || output.type === 'temp') {
          ...
          radio_button_img = $el(
            'img',
            { src: '', style: { width: 'auto', height: '100px' } },
            []
          )
        }
