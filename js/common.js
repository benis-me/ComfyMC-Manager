...
export async function install_checked_custom_node(
  grid_rows,
  target_i,
  caller,
  mode
  ...
  if (caller) {
    ...
    for (let i in grid_rows) {
      if (!grid_rows[i].checkbox.checked && i != target_i) continue
    ...
    caller.updateMessage(
      "<BR>To apply the installed/updated/disabled/enabled custom node, please <button id='cm-reboot-button' class='cm-small-button'>RESTART</button> ComfyUI. And refresh browser.",
      'cm-reboot-button'
    )
  }
}
...
export async function install_via_git_url(url, manager_dialog) {
  ...
  if (res.status == 200) {
    app.ui.dialog.show(
      `'${url}' is installed<BR>To apply the installed custom node, please <button id='cm-reboot-button'><font size='3px'>RESTART</font></button> ComfyUI.`
    )
