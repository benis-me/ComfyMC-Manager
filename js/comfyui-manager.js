...
function drawBadge(node, orig, restArgs) {
  ...
  if (
    ...
    let text = ''
    if (badge_mode.startsWith('id_nick')) text = `#${node.id} `
...
async function fetchUpdates(update_check_checkbox) {
  ...
  try {
    ...
    const response = await api.fetchApi(
      `/customnode/fetch_updates?mode=${mode}`
    )
    ...
    if (response.status == 201) {
      app.ui.dialog.show(
        "There is an updated extension available.<BR><BR><P><B>NOTE:<BR>Fetch Updates is not an update.<BR>Please update from <button id='cm-install-customnodes-button'>Install Custom Nodes</button> </B></P>"
      )
      ...
      button.addEventListener('click', async function () {
        ...
        await CustomNodesInstaller.instance.show(
          CustomNodesInstaller.ShowMode.UPDATE
        )
      })
...
async function updateAll(update_check_checkbox, manager_dialog) {
  ...
  try {
    ...
    if (response1.status != 200 && response2.status != 201) {
      app.ui.dialog.show(
        'Failed to update ComfyUI or several extensions.<BR><BR>See terminal log.<BR>'
      )
      app.ui.dialog.element.style.zIndex = 10010
      return false
    }
    if (response1.status == 201 || response2.status == 201) {
      app.ui.dialog.show(
        "ComfyUI and all extensions have been updated to the latest version.<BR>To apply the updated custom node, please <button class='cm-small-button' id='cm-reboot-button'>RESTART</button> ComfyUI. And refresh browser."
      )
    ...
    app.ui.dialog.show(
      `Failed to update ComfyUI or several extensions / ${exception}`
    )
    app.ui.dialog.element.style.zIndex = 10010
    return false
  } finally {
...
class ManagerMenuDialog extends ComfyDialog {
  createControlsMid() {
    ...
    fetch_updates_button = $el('button.cm-button', {
      type: 'button',
      textContent: 'Fetch Updates',
      onclick: () => fetchUpdates(this.update_check_checkbox),
    })

    update_all_button = $el('button.cm-button', {
      type: 'button',
      textContent: 'Update All',
      onclick: () => updateAll(this.update_check_checkbox, self),
    })
      ...
      $el('button.cm-button', {
        ...
        onclick: () => {
          ...
          CustomNodesInstaller.instance.show(
            CustomNodesInstaller.ShowMode.NORMAL
          )
        },
      }),
      ...
      $el('button.cm-button', {
        ...
        onclick: () => {
          if (!CustomNodesInstaller.instance)
            CustomNodesInstaller.instance = new CustomNodesInstaller(app, self)
          CustomNodesInstaller.instance.show(
            CustomNodesInstaller.ShowMode.MISSING_NODES
          )
        },
      }),
      ...
      $el('button.cm-button', {
        ...
        onclick: () => {
          if (!AlternativesInstaller.instance)
            AlternativesInstaller.instance = new AlternativesInstaller(
              app,
              self
            )
  ...
  createControlsLeft() {
    ...
    this.update_check_checkbox = $el(
      'input',
      { type: 'checkbox', id: 'skip_update_check' },
      []
    )
    const uc_checkbox_text = $el('label', { for: 'skip_update_check' }, [
      ' Skip update check',
    ])
    ...
    this.datasrc_combo.appendChild(
      $el('option', { value: 'cache', text: 'DB: Channel (1day cache)' }, [])
    )
    this.datasrc_combo.appendChild(
      $el('option', { value: 'local', text: 'DB: Local' }, [])
    )
    this.datasrc_combo.appendChild(
      $el('option', { value: 'url', text: 'DB: Channel (remote)' }, [])
    )

    // preview method
    let preview_combo = document.createElement('select')
    preview_combo.className = 'cm-menu-combo'
    preview_combo.appendChild(
      $el('option', { value: 'auto', text: 'Preview method: Auto' }, [])
    )
    preview_combo.appendChild(
      $el(
        'option',
        { value: 'taesd', text: 'Preview method: TAESD (slow)' },
        []
      )
    )
    preview_combo.appendChild(
      $el(
        'option',
        { value: 'latent2rgb', text: 'Preview method: Latent2RGB (fast)' },
        []
      )
    )
    preview_combo.appendChild(
      $el(
        'option',
        { value: 'none', text: 'Preview method: None (very fast)' },
        []
      )
    )
    ...
    badge_combo.appendChild(
      $el('option', { value: 'none', text: 'Badge: None' }, [])
    )
    badge_combo.appendChild(
      $el('option', { value: 'nick', text: 'Badge: Nickname' }, [])
    )
    badge_combo.appendChild(
      $el(
        'option',
        { value: 'nick_hide', text: 'Badge: Nickname (hide built-in)' },
        []
      )
    )
    badge_combo.appendChild(
      $el('option', { value: 'id_nick', text: 'Badge: #ID Nickname' }, [])
    )
    badge_combo.appendChild(
      $el(
        'option',
        { value: 'id_nick_hide', text: 'Badge: #ID Nickname (hide built-in)' },
        []
      )
    )
    ...
    api
      .fetchApi('/manager/channel_url_list')
        ...
        try {
          ...
          for (let i in urls) {
            if (urls[i] != '') {
              ...
              channel_combo.appendChild(
                $el(
                  'option',
                  { value: name_url[0], text: `Channel: ${name_url[0]}` },
                  []
                )
              )
            }
          ...
          channel_combo.addEventListener('change', function (event) {
            api.fetchApi(
              `/manager/channel_url_list?value=${event.target.value}`
            )
          })
    ...
    for (const option of share_options) {
      share_combo.appendChild(
        $el('option', { value: option[0], text: `Share: ${option[1]}` }, [])
      )
    }
    ...
    return [
      ...
      $el('button.cm-button', {
        ...
        onclick: () => {
          var url = prompt(
            'Please enter the URL of the Git repository to install',
            ''
          )
  ...
  createControlsRight() {
    const elts = [
                ...
                [
                  {
                    ...
                    callback: () => {
                      ...
                      app
                        ...
                        .then((nodes) => {
                          // console.log({ nodes });
                          const { potential_outputs, potential_output_nodes } =
                            getPotentialOutputsAndOutputNodes(nodes)
                          ...
                          if (potential_outputs.length === 0) {
                            if (potential_output_nodes.length === 0) {
                              ...
                              const supported_nodes_string =
                                SUPPORTED_OUTPUT_NODE_TYPES.join(', ')
                              alert(
                                `No supported output node found (${supported_nodes_string}). To share this workflow, please add an output node to your graph and re-run your prompt.`
                              )
                            } else {
                              alert(
      ...
      $el('button.cm-button', {
        ...
        onclick: () => {
          window.open('https://ltdrdata.github.io/', 'comfyui-node-info')
        },
      }),
      $el('br', {}, []),
    ]
...
app.registerExtension({
  ...
  async setup() {
    ...
    managerButton.onclick = () => {
      if (!manager_instance) setManagerInstance(new ManagerMenuDialog())
  ...
  async loadedGraphNode(node, app) {
    if (!node.badge_enabled) {
      ...
      node.getNickname = function () {
        return getNickname(node, node.type.trim())
      }
      node.onDrawForeground = function (ctx) {
        drawBadge(node, orig, arguments)
      }
