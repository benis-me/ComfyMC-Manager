...
async function getAlterList() {
  ...
  const response = await api.fetchApi(
    `/alternatives/getlist?mode=${mode}${skip_update}`
  )
...
export class AlternativesInstaller extends ComfyDialog {
  ...
  apply_searchbox(data) {
    ...
    for (let i in this.grid_rows) {
      ...
      let content =
        data1.tags.toLowerCase() +
        data1.description.toLowerCase() +
        data2.author.toLowerCase() +
        data2.description.toLowerCase() +
        data2.title.toLowerCase()
    ...
    if (this.data)
      for (var i = 0; i < this.data.length; i++) {
        ...
        checkbox.addEventListener('change', function () {
          self.invalidate_checks.call(
            self,
            checkbox.checked,
            data.custom_node?.installed
          )
        })
          ...
          if (installBtn2 != null) {
            ...
            installBtn2.addEventListener('click', function () {
              install_checked_custom_node(
                self.grid_rows,
                j,
                AlternativesInstaller.instance,
                'update'
              )
            })
  ...
  createHeaderControls() {
    ...
    this.search_box = $el(
      ...
      {
        type: 'text',
        id: 'manager-alternode-search-box',
  ...
  async createBottomControls() {
    ...
    this.message_box = $el('div', { id: 'alternatives-installer-message' }, [
      $el('br'),
      '',
    ])
